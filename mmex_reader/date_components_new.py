"""Date Components for the MMEX Kivy application.

This module provides date-related UI components including date pickers.
"""
...
from ui_config_new import ui_config, create_popup, show_popup
    ...
    def _on_date_selected(self, selected_date: Optional[str]) -> None:
        ...
        try:
            if selected_date:
                ...
                self.button.text = self.current_date
                
                # Call the date change callback if provided
                if self.date_change_callback:
                    self.date_change_callback(self, self.current_date)
                    
        except Exception as e:
            logger.error(f"Error handling date selection: {e}")
            self.show_error("Error processing date selection")
