    ...
    try:
        ...
        if df is None or df.empty:
            ...
            no_data_label = Label(
                ...
                size_hint_y=None,
                height=30,
                color=(0, 0, 0, 1)  # Default black text
            )
        ...
        # PERFORMANCE OPTIMIZATION: Temporarily disable grid's size and pos calculations during bulk addition
        grid.disabled = True  # Prevent visual updates during bulk operations
...
def _add_grid_data_rows(grid, df, display_headers, row_click_callback, ui_config):
    ...
    try:
        ...
        all_widgets = []
        num_columns = len(display_columns) if display_columns else len(df.columns) if not df.empty else 1
        col_width_hint = 1.0 / num_columns if num_columns > 0 else 1.0

        # Pre-calculate text formatting to reduce processing inside the loop
        formatted_data = []
        for row_index, row in df.iterrows():
            ...
            for col in display_columns:
                ...
                formatted_text = _format_cell_value(col, value)
                formatted_row.append((formatted_text, row.to_dict()))
            formatted_data.append(formatted_row)

        for formatted_row in formatted_data:
            for text, row_dict in formatted_row:
                if row_click_callback:
                    ...
                    cell_widget.bind(on_press=make_on_row_click(row_dict))
                else:
                    # Create regular label
                    cell_widget = _create_data_label(text, num_columns)

                all_widgets.append(cell_widget)

        # PERFORMANCE OPTIMIZATION: Set grid height binding before adding widgets to avoid recalculation
        ...
        grid.add_widget = grid.add_widget  # Cache the method for performance
        for widget in all_widgets:
            grid.add_widget(widget)

        # Set the grid height to the calculated minimum to ensure proper display
